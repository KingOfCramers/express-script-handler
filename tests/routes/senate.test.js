...
describe("Testing query of Senate routes", () => {
  ...
  test("Query all data", async done => {
    ...
    let res = await request(app).get("/data/committees/senate/sfrcs");
  ...
  test("Disallow improper collection", async done => {
    let res = await request(app).get("/data/committees/senate/thisisnotreal");
  ...
  test("Query by type", async done => {
    // Query MongoDB and Check data...
    let res = await request(app).get("/data/committees/senate/sfrcs?title=Strengthening Security and the Rule of Law in Mexico");
    expect(res.statusCode).toBe(200);
    expect(Array.isArray(res.body));
    ...
    expect(detail).toBe("hearing");
    // Ensure the query was saved to the Redis database...
    let redisData = JSON.parse(await client.hget('sfrcs', '{"title":"Strengthening Security and the Rule of Law in Mexico"}'));
    expect(Array.isArray(redisData));
    expect(redisData[0].type === detail);
    done();
  });
  ...
  test("Don't allow POST", async done => {
    // Attempt post route.
    let res = await request(app).post("/data/committees/house/sfrcs");
    expect(res.statusCode).toBe(404);
